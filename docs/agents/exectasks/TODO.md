...
- Task: デザインガイドを実装レベルで利用・守れるように整備する
  - frontend-development skillを作成し、その中にUI designが必要な場合は./design.mdを追加で読むこと、といった誘導をし、design.mdに実装上守るべき内容を記載する。docs/guide/ui-design.mdへのリンクとしてもよい。 /skill-dev skill を利用して実装すること
  - local-reviewer agentのレビュー指針に「変更範囲に対応したskillを参照して、それを守っているかをレビューするようにする」という内容を盛り込み、デザインガイドを守った変更になっているかレビュー出来るようにする
- Task: CSSにリファクタリングした方が良い場所があるかを検討し、あればリファクタリングTaskを追加する
  - CSSの実装とデザインガイドを読み込んで、デザインガイドに即した実装をしやすい構造が作れないかを検討するのが良いかも（指示者も疎いのでより良い洞察に向けた切り口があればやってもらってかまわない）
