...
- Task: バツボタンが少し小さい・バツボタンを押したら「保存しました」と言われるが、保存を1要素ごとに出来るようにするんじゃなくて保存ボタンを1つにしたらチグハグな動作じゃなくなりそう・フォームが空・変更なしで保存ボタンをdisableしたい・保存しましたトーストが連続で発生したときにフィードバックがなくなるのをなんとかしたい
