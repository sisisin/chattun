...
4. 変更されたファイルのパスに基づき、関連するスキルを読んで規約を把握する
   - 例: `front/src/` 配下のファイルが変更されていれば `.claude/skills/frontend-development/` のスキルを読み、UIデザインガイド（design.md）の規約も確認する
   - スキルに定義されたルール（セマンティックトークンの使用、スペーシング規約等）への準拠もレビュー対象とする
